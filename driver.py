    ...
    try:
        ...
        for device in computer.Win32_PnPEntity():
            if device.Name and device.DeviceID:
                status = "Yüklü" if device.Status == "OK" else "Yüklü Değil" #cihaz durumunu 
                driver_info={
                    "Cihaz": device.Name,
        ...
        if save_to_file: 
            ...
            with open(file_path,"w",encoding="utf-8") as file:
                json.dump(drivers,file,ensure_ascii=False, indent=4) #5 veri ıcın 4 bosluk 
...
def display_drivers_in_table():
    ...
    if not drivers:  
        print("Sürücü bilgileri alınamadı.")
        return
...
#tkinter araryuz
    ...
    def search_drivers(event=None):
       ...
       filtered_drivers = [
            driver for driver in drivers #makinenin sectigi driver icinde bizim driver var mı onu kntrol edicez
